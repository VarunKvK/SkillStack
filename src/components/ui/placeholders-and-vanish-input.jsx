...
export function PlaceholdersAndVanishInput({
  ...
  const [error, setError] = useState(null); // Error state for the validation message
  const form = useForm({
  ...
  return (
    <Form {...form}>
      <form
        ...
        onSubmit={form.handleSubmit(handleFormSubmit)} // Hook the form with react-hook-form
      >
        <canvas
          className={cn(
        ...
        <div className="relative flex items-end">
          <div className="w-full">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <Textarea
                    {...field}
                    {...register("description")} // Register the textarea for validation
                    onChange={handleInputChange}
                    onKeyDown={handleKeyDown}
                    ref={inputRef}
                    value={value}
                    className={cn(
                      "w-full relative text-sm sm:text-base z-50 border-none dark:text-white bg-transparent text-black h-full focus:outline-none focus:ring-0 pl-4 pt-3",
            ...
            />
            <div className="absolute inset-0 flex mt-[0.65rem] pointer-events-none">
